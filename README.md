...
**Release [`v0.2.0`](https://github.com/azzindani/MCP_Web_Browser/releases/tag/v0.2.0)** — source only. No wheel and no container image are published: install from the tag with the bundled installer, or build the image yourself from the `Dockerfile` in this repo.
...
|---|---|---|
| `MCP_DATA_ROOT` | current working directory | Root for `*.db`, `*.jsonl` |
...
`MCP_OUTPUT_DIR` deliberately governs **only** exports, not `MCP_DATA_ROOT`.
The crawl DB resolves through the same path guard, so folding the two
together relocates `krawl.db` into the shared directory and you come back to
an empty index. The traversal guard applies to both roots either way.
